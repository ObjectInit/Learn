#region  <<版本注释>>
/* ==========================================================
// <copyright file="AcReceiptD.cs" company="Shiji.BO.CS">
// Copyright (c) Shiji.BO.CS. All rights reserved.
// </copyright>
* 功能描述：收款体
* 创 建 者：曾倩倩
* 创建时间：2019/9/26 16:53:28
* =============================================================*/
#endregion
...
namespace SJ.BackEnd.Biz.AC
{
    /// <summary>
    /// 收款体
    /// 1 描述：在应收应付指定的科目对账中，根据应收的凭证分录生成的收款单据
    /// 2 约定：
    ///     1.新增时使用parent存储父凭证分录id，科目固定为z90001.收款单；
    ///     2.修改时使用after标识跳过数据校验
    /// 3 业务逻辑：
    ///     新增前：初始化数据父凭证的单据体、对方科目单据体数据
    ///     新增后：新增对方科目单据体后，更新单据头的借贷合计和对方科目的贷方金额
    ///     修改前：
    ///         1.根据状态判断单据数据是否能修改，设置能够修改的属性；
    ///         2.校验存在父凭证分录的收款体数据是否合法；
    ///         3.对方科目无变化时，全面校验；发生变化时，清空所有tcode、数量信息、外币信息，并简单校验数据
    ///     修改后：修改存在父凭证的单据体的实收金额后，更新单据头的借贷合计和对方科目的贷方金额
    /// </summary>
    [ClassData("cname", "收款体", "vision", 1)]
    public class AcReceiptD : BizTableVoucherD
    {
        /// <summary>
        /// 预留字段、子查询定义
        /// </summary>
        protected override void OnCustomDefine()
        {
            ...
            ((FieldDefine)ListField["amountd"]).DisplayName = this.ApiTask.L("实收金额");
            ...
            // 更改头的关联对象
            ((FieldDefine)ListField["vh"]).RefBiz = "AcReceipt";
        ...
        /// 1.设置科目为收款科目
        /// </summary>
        public override SData BaseParms => new SData("account", $"[{ConstResourceDB.GetAccount(this.ApiTask, AcConst.ReceiptAccount)}]");
        ...
        protected override void OnInsertBefore(SData data)
        {
            // 判断逻辑 **********
            // * 存在父凭证的单据体，校验必填（实收金额）
            if (data["cd"].ToString() == AcConst.Debit.ToString())
            {
                this.CheckEmptyOrNull(data, true, "amountd");
            }
        ...
        /// 修改数据前
        /// 1、已审的单据不允许修改：“所选记录已审”
        /// 2、修改存在父凭证的单据体后，修改对方科目单据体的贷方金额，不需要做数据校验
        /// 3、修改单据头核算期后修改单据体的核算期，不需要做数据校验
        /// 4、存在父凭证的单据体，只允许修改实收金额，草稿状态下，同时修改提审金额
        /// 5、对方科目单据体，忽略修改金额、科目对账、核算期、科目、核算单位、父凭证，待审状态下，只允许修改摘要
        /// 6、校验数据：
        ///             1、初始化数据；
        ///             2、存在父凭证的单据体，校验是否允许收款；
        ...
        protected override void OnUpdateBefore(SData oldData, SData updateData)
        {
            ...
            if (!updateData.ContainsKey("isapprove"))
            {
                ...
                // * 修改存在父凭证的单据体后，修改对方科目单据体的贷方金额，不需要做数据校验
                    ...
                    && updateData["after", string.Empty].ToString() == "操作单据体金额后修改对方科目单据体")
                {
                    ...
                    var dBiz = this.ApiTask.Biz<BizTable>("AcReceipt"); // 收款头对象
                    var receipt = dBiz.GetItemByParms(new SData("id", oldData["vh.id"]).toParmStr(), "account.isqty,account.iscurrency");
                    SData subject = null;
                    if (!string.IsNullOrEmpty(receipt.Item<string>("account.isqty")))
                    {
                        if (!string.IsNullOrEmpty(receipt.Item<string>("account.iscurrency")))
                        {
                            subject = new SData("isqty", receipt["account.isqty"], "iscurrency", receipt["account.iscurrency"]);
                ...
                // * 存在父凭证的单据体，只允许修改实收金额，草稿状态下，同时修改提审金额
                if (oldData["cd"].ToString() == AcConst.Debit.ToString())
                {
                    // 是否修改实收金额
                    bool isChangeAmount = updateData.ContainsKey("amountd");
                    object updateAmount = updateData["amountd"];
                    updateData.Clear();

                    if (isChangeAmount)
                    {
                        updateData["id"] = oldData["id"];
                        updateData.Add("amountd", updateAmount);

                        // 单据状态为草稿：修改实收金额时需要同时修改提审金额
                ...
                if (updateData.Count > 0)
                {
                    ...
                    if (oldData["cd"].ToString() == AcConst.Debit.ToString())
                    {
                        // 判断逻辑 **********
                        // 校验必填
                        this.CheckEmptyOrNull(updateData, false, "amountd");

                        // 业务逻辑 **********
                        // 简单校验是否允许收款
                        updateData["subrec"] = oldData["subrec"];
                        updateData["parent"] = oldData["parent"];
                        VoucherHelper.CheckCanPayReceipt(oldData, updateData, this, false);
                    }
                    else // 对方科目单据体
                    {
                        // 判断逻辑 **********
                        ...
                        var biz = this.ApiTask.Biz<BizTable>("AcReceipt");
                        var receipt = biz.GetItemByParms(new SData("id", oldData["vh.id"]).toParmStr(), "account.id,account.dcode");
                        if (receipt != null)
                        {
                            // 得到前端传入的修改前的对方科目
                            ...
                            if (!string.IsNullOrEmpty(oldAccount))
                            {
                                ...
                                int now = -2;
                                if (!string.IsNullOrEmpty(receipt.Item<string>("account.id")))
                                {
                                    int.TryParse(receipt["account.id"].ToString(), out now);
                                }
                                ...
                                else // 科目发生变化时，清空所有tcode、数量信息、外币信息，对科目做特殊处理，并简单校验数据
                                {
        ...
        protected override void OnUpdateAfter(SData oldData, SData updateData, SData modify)
        {
            ...
            if (!updateData.ContainsKey("isapprove"))
            {
                if (oldData["cd"].ToString() == AcConst.Debit.ToString())
                {
                    if (modify.ContainsKey("amountd"))
                    {
                        // * 更新单据头的借贷合计和对方科目的贷方金额
                        ChangeAmount(oldData.Item<string>("vh.id"));

                        // * 更新勾对记录的金额
                        var recodBiz = ApiTask.Biz<BizTableVoucherD>(nameof(AcRecoD));
                        var recod = recodBiz.GetItemByParms(new SData("recpay", oldData["vh.id"], "parent", oldData["parent"]).toParmStr(), "id");
                        if (recod != null)
                        {
                            recodBiz.Update(new SData("id", recod["id"], "amountd", updateData["amountd"], "recoamount", updateData["amountd"]));
                        }
                    }
                }
            }
        ...
        private void ChangeAmount(string vh)
        {
            if (!string.IsNullOrEmpty(vh))
            {
                // 数据准备 **********
                // 变量声明
                // 聚合查询得到单据头下存在父凭证的单据体的实收金额和
                object amountd = this.GetAggregate(new SData("vh", $"[{vh}]", "cd", AcConst.Debit).toParmStr(), EAggregate.Sum, "amountd");
                decimal sumc = 0;
                if (amountd != null)
                {
                    decimal.TryParse(amountd.ToString(), out sumc);
                }

                // 业务逻辑 **********
                // * 修改单据头的借贷合计
                var dBiz = this.ApiTask.Biz<BizTable>("AcReceipt");
                dBiz.Update(new SData("id", vh, "sumc", sumc, "sumd", sumc, "after", "操作单据体金额后修改单据头"));

                // * 修改对方科目的贷方金额
                var subjectD = this.GetItemByParms(new SData("vh", $"[{vh}]", "cd", AcConst.Credit).toParmStr(), "id");
                if (subjectD != null)
                {
                    this.Update(new SData("id", subjectD["id"], "amountd", 0, "amountc", sumc, "after", "操作单据体金额后修改对方科目单据体"));
                }
            }
        }
