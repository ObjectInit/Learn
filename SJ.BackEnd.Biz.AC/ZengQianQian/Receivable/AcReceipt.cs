#region  <<版本注释>>
/* ==========================================================
// <copyright file="AcReceipt.cs" company="Shiji.BO.CS">
// Copyright (c) Shiji.BO.CS. All rights reserved.
// </copyright>
* 功能描述：收款头
* 创 建 者：曾倩倩
* 创建时间：2019/9/26 16:53:15
* =============================================================*/
#endregion
...
namespace SJ.BackEnd.Biz.AC
{
    ...
    /// 2 约定：
    ///     1.新增时使用dList存储选择收款的凭证分录，分录中使用parent存储凭证分录id、amountd存储实收金额；
    ///     2.使用subjectD存储对方科目收款体，使用account存储对方科目
    /// 3 业务逻辑：
    ///     新增前：核算期校验，校验是否允许收款，屏蔽父类方法（避免占用凭证的编号）
    ...
    /// </summary>
    [ClassData("cname", "收款头", "ApproveFlow", "z03", "vision", 1)]
    public class AcReceipt : AcPayReceipt
    {
        /// <summary>
        /// 预留字段、参数、子查询定义
        /// </summary>
        protected override void OnCustomDefine()
        {
            ...
            ((FieldDefine)ListField["sumc"]).DisplayName = this.ApiTask.L("实收金额");

            ((FieldDefine)ListField["vtype"]).DisplayName = this.ApiTask.L("凭证类型");
            ((FieldDefine)ListField["period"]).DisplayName = this.ApiTask.L("核算期");

            // 设置对象名和中文名审批流程使用
            this.BizD = "AcReceiptD";
            this.BizName = "收款";

            // 查询参数定义 **********
            // 根据勾对体Id查找收款单头集合(勾对体Id可以多个)
            this.AddParm("recodIds", "勾对Id");

            // 子查询定义 **********
            // 收款体子查询
            this.AddSubQuery("detail", "收款体", "AcReceiptD", "vh=id");
...
exists (select 1 
    ...
    vd.id in ({parmStr}))";
        ...
        protected override void OnInsertBefore(SData data)
        {
            // 参数定义 **********
            // * 初始化数据
            VoucherHelper.SetInitData(data, this.ApiTask, nameof(AcReceipt));
            ...
            // * 校验是否允许收款
            VoucherHelper.CheckCanPayReceipt(data, this, false, false);
        }
        ...
        protected override void OnInsertAfter(SData data)
        {
            // 数据准备 **********
            // 变量声明
            decimal sum = 0; // 实收金额合计
            var vh = $"[{data["id"]}]"; // 单据头id
            var dBiz = this.ApiTask.Biz<BizTable>("AcReceiptD"); // 收款体对象
            var dList = data["dList"] as List<SData>; // 选择收款的凭证分录列表
            var accountBiz = this.ApiTask.Biz<BizTableCode>("acaccount"); // 科目对象
            int accountId = accountBiz.GetIDAuto(AcConst.ReceiptAccount); // 收款科目
            ...
            {
                item["vh"] = vh;
                item["cd"] = AcConst.Debit;
                item["amountc"] = 0; // 贷方金额为0
                ...
                decimal.TryParse(item.Item<string>("amountd"), out decimal amountd);
                sum += amountd;

                // 准备添加的勾对记录列表（勾对金额、科目对账、借贷、逻辑标志、父id、状态、子状态、数据类型、核算单位、付款体id、核算期）
                var tick = new SData("recoamount", amountd, AcConst.RecoTcode, $"[{data["subrec.id"]}]", "cd", voucher["cd"], "logicsign", AcConst.RecoVoucher, "parent", voucher["id"], "vstate", $"[{data["vstate.id"]}]", "vsubstate", $"[{data["vsubstate.id"]}]", "vclass", $"[{data["vclass.id"]}]", "SubUnit", $"[{data["SubUnit.id"]}]", "recpay", data["id"], "period", data["period"]);
                if (firstTick == null)
                {
                    firstTick = tick;
                }
            ...
            subjectD["vh"] = vh;
            subjectD["cd"] = AcConst.Credit;
            subjectD["parent"] = string.Empty;
            subjectD["amountd"] = 0; // 借方金额为0
            subjectD["amountc"] = sum; // 贷方金额为实收金额和
        ...
        protected override void OnDeleteBefore(SData data)
        {
            ...
            // * 删除单据头前先删除单据体的数据
            var dBiz = ApiTask.Biz<BizTable>("AcReceiptD");
            recDList = dBiz.GetListData(0, 1, new SData("vh", $"[{data["id"]}]").toParmStr(), "id,parent");
        ...
        /// 付款单体列表
        /// </summary>
        List<SData> recDList = new List<SData>();
        ...
        protected override void OnDeleteAfter(SData Data)
        {
            // * 反复核父凭证
            var dBiz = ApiTask.Biz<BizTable>("AcReceiptD");
            var recDIdList = recDList.Select(x => x.Item<int>("id")).ToList();
            var vdIdList = recDList.Select(x => x.Item<int>("parent")).ToList();
            AcVoucherHelper.UpdateVSubstateNoLock(this, vdIdList, recDIdList);
        ...
        protected override void OnUpdateBefore(SData oldData, SData updateData)
        {
            ...
            if (!updateData.ContainsKey("isapprove"))
            {
                ...
                // * 操作单据体金额后修改单据头的借贷合计，不需要做数据校验
                if (updateData.ContainsKey("sumc")
                    && updateData.ContainsKey("sumd")
                    && updateData["after", string.Empty].ToString() == "操作单据体金额后修改单据头")
                {
                    // 实收金额校验（不能为空，不能为0，是数值）
                    AcVoucherHelper.CheckDecimal(this.ApiTask, updateData.Item<string>("sumc"), ((FieldDefine)ListField["sumc"]).DisplayName);
        ...
        protected override void OnUpdateAfter(SData oldData, SData updateData, SData modify)
        {
            ...
            if (modify.ContainsKey("period"))
            {
                ...
                var dBiz = ApiTask.Biz<BizTable>("AcReceiptD"); // 收款体对象
